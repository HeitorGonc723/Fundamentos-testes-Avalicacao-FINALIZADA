...
function calcularPedido(pedido: IPedido): IResultadoPedido {
    ...
    for (const itemPedido of pedido.itens) {
        const item = cardapio.find(c => c.id === itemPedido.itemId)
        ...
        if (item) { 
            ...
            if (item.categoria === 'prato') Prato = true
            if (item.categoria === 'bebida') Bebida = true
            if (item.categoria === 'sobremesa') Sobremesa = true
        }
        
    }

    if (totalItens > 20 || subtotal < 25) {
        return {
            subtotal: subtotal,
            desconto: 0,
            taxaEntrega: 0,
    ...
    const ehCombo = Prato && Bebida && Sobremesa
    let desconto = 0
    if (ehCombo) {
    ...
    let taxaEntrega = subtotal > 100 ? 0 : 8
