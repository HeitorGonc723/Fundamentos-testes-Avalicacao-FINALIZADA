...
function calcularPlano(plano: string, personal: boolean): IResultadoPlano {
    ...
    const custoPersonal = personal ? 50 * meses : 0
    const valorMensal = (baseCusto / meses) + (personal ? 50 : 0)
    const valorTotal = baseCusto + custoPersonal
...
function realizarCheckin(checkin: ICheckin): IResultadoCheckin {
    ...
    const jaCheckin = checkIns.some(c => c.alunoId === checkin.alunoId && c.data === dataCheckin)
    if (jaCheckin) {
        return {
            permitido: false,
            mensagem: 'Check-in já realizado hoje'
        }
    }
    checkIns.push({
...
function cancelarPlano(alunoId: number): IResultadoCancelamento {
    ...
    const hoje = new Date()
    const diferenca = aluno.vencimento.getTime() - hoje.getTime()
    const diasDiferenca = diferenca / (1000 * 60 * 60 * 24)
    const mesesVencidos = Math.ceil(diasDiferenca / 30)
    if (mesesVencidos <= 0) {
        return {
            multa: 0,
            ehValido: true
    ...
    const valorMensal = calcularPlano(aluno.plano, aluno.personal).valorMensal
    const valorRestante = mesesVencidos * valorMensal
